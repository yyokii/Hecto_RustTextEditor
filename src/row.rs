            ...
            .take(end - start)
        {
            if let Some(c) = grapheme.chars().next() {
                ...
                if highlighting_type != current_highlighting {
                    ...
                    let start_highlight = format!("{}", highlighting_type.to_color_sequence());
                ...
                if *next_char == '*' {
                    let closing_index =
                        if let Some(closing_index) = self.string[*index + 2..].find("*/") {
                            *index + closing_index + 4 // 「/*」の次（+2）からfindしているので+2し、また最後の「*/」も含みたいので+2する。
                        } else {
                            chars.len()
                        };
    ...
    pub fn highlight(
        ...
        if self.is_highlighted && word.is_none() {
            if let Some(hl_type) = self.highlighting.last() {
                if *hl_type == highlighting::Type::Comment
                    && self.string.len() > 1
